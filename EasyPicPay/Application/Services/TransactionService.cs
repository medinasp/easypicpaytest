    ...
    : ITransactionService
{
    ...
    public async Task<bool> AuthorizeTransactionAsync(Guid transactionId)
    {
        // Simulação de autorização externa (serviço mock)
        await Task.Delay(100); // Simula latência
        var random = new Random();
        return random.Next(0, 10) > 2; // 80% de sucesso
    }

    public async Task CompleteTransactionAsync(Guid transactionId, string authorizationCode)
    {
    ...
    public async Task FailTransactionAsync(Guid transactionId, string reason)
    {
        await using var transactionScope = await context.Database.BeginTransactionAsync();
        ...
        try
        {
            ...
            // Reverte valores se a transação foi debitada
            if (transaction.Status == nameof(TransactionStatus.Pending))
            {
                if (transaction.Payer != null)
                    transaction.Payer.Credit(transaction.Amount);
                    
                if (transaction.Payee != null)
                    transaction.Payee.Debit(transaction.Amount);
            }
        ...
        {
            await transactionScope.RollbackAsync();
            logger.LogError(ex, "Erro ao falhar transação {TransactionId}", transactionId);
            throw;
        }
    }
